...
void spawn_axe(void) {
    ...
    for (int i = 0; i < enemy_count; i++) {
        if (enemies[i].active && ((player.pos.x == enemies[i].pos.x && player.pos.y == enemies[i].pos.y) || (player2.pos.x == enemies[i].pos.x && player2.pos.y == enemies[i].pos.y))) {
            game_active = 0;
            player_won = 0;
            return;
        }
...
if "handle_player_caught" not in content:
    catch_old = """        // Evaluate if Beast caught the Caveman after moving
        if ((enemies[e].pos.x == player.pos.x && enemies[e].pos.y == player.pos.y) || (enemies[e].pos.x == player2.pos.x && enemies[e].pos.y == player2.pos.y)) {
            game_active = 0;
            player_won = 0;
        }"""
